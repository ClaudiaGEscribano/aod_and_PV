...
inputfile <- "../../data/C-AER/rsds_day_20032009.nc"
...
rsds <- stack(inputfile, varname="rsds")
# Fix the projection and extent
projection(rsds) <- mycrs
extent(rsds) <- extent(plonlat) 
# Take a look
rsds
plot(rsds)
...
r <- projectRaster(s, crs=CRS("+proj=longlat +datum=WGS84"))
