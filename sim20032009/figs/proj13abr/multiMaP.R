...
pdf("useTday/RelDif_aer_no_all20032009SIG4.pdf", height=6, width=6)
levelplot(s1, scales=list(draw=FALSE), ylab=list(c("SON", "JJA", "MAM", "DJF"), rot=0, cex=0.7), xlab=list(c("Fixed", "One", "Two"),cex=0.7), names.attr=c(rep('', 12)), at=my.at, layout=c(3,4))+ layer(sp.lines(border, lwd=0.3))+
    ...
    layer(sp.text(coordinates(labsLat),
                  ...
                  adj = c(-0.25, -0.25),
                  cex = 0.2))+
    layer(sp.points(pvpointsDJF[[1]], pch=20, cex=0.1, alpha=0.5,lwd=0.2), columns=1, rows=1)+
    layer(sp.points(pvpointsDJF[[2]], pch=20, cex=0.1, alpha=0.5, lwd=0.2), columns=2, rows=1)+
    layer(sp.points(pvpointsDJF[[3]], pch=20, cex=0.1, alpha=0.5, lwd=0.2), columns=3, rows=1)+
